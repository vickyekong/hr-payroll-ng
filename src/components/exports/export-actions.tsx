...
export function ExportActions({
  ...
  const [loading, setLoading] = useState<"download" | "drive" | null>(null);
  ...
  async function uploadToDrive() {
    if (!driveConnected) {
      alert("Connect Google Drive in Settings first.");
      return;
    }
    setLoading("drive");
    try {
      ...
      if (data.webViewLink) {
        const open = confirm(
          `Uploaded ${data.filename} to Google Drive.\n\nOpen the file now?`
        );
  ...
  return (
    <div className="flex flex-wrap gap-2">
      ...
      <Button
        ...
        disabled={loading !== null || (kind === "payroll" && !runId)}
      >
        {loading === "drive" ? "Uploading…" : "Save to Google Drive"}
      </Button>
    </div>
  );
}
